...
const deletionDigraphDisplayLogic = function (valueOfNodeToDelete) {
  ...
  if (deletionNode.left === null && deletionNode.right === null) {
    ...
    template.push(`"${deletedNodeParent.value}" -> ${deletionNode.left.value} ${GRAPHVIZ_CONFIG.deletionActionStyles.newChildNode(deletedNodeParent.value, replacementNode.value)}`)
